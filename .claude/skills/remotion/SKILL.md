---
name: remotion
description: Build programmatic videos with Remotion (React-based video rendering). Use when creating or editing video compositions, animating React components frame-by-frame, working with src/Root.tsx / <Composition>, or using Remotion APIs like useCurrentFrame, interpolate, spring, Sequence, Series, TransitionSeries, AbsoluteFill, OffthreadVideo, Img, Audio, Gif, or staticFile. Triggers on .tsx files in a Remotion project and any request to make/animate/render a video in React.
---
...
If a video is included it should use the `<OffthreadVideo>` tag.
...
- `OffthreadVideo` has an `endAt` prop that limits how long a video is shown.
- `OffthreadVideo` has a `volume` prop that sets the volume of the video (values between 0 and 1).

If a non-animated image is included it should use the `<Img>` tag.
...
If an animated GIF is included, install the `@remotion/gif` package and use the `<Gif>` tag.
...
If audio is included, use the `<Audio>` tag.
...
Asset sources can be either a remote URL or an asset referenced from the `public/` folder of the project. Assets referenced from `public/` must use the `staticFile` API from Remotion.
...
- `Audio` has a `startFrom` prop that trims the left side of audio by a number of frames.
- `Audio` has an `endAt` prop that limits how long audio is shown.
- `Audio` has a `volume` prop that sets the volume of the audio (values between 0 and 1).

If two elements should be rendered on top of each other, layer them using the `AbsoluteFill` component from `remotion`.
...
- A `Sequence` has a `from` prop that specifies the frame number where the element should appear. `from` can be negative, in which case the Sequence starts immediately but cuts off the first `from` frames.
...
For displaying multiple elements one after another, use the `Series` component from `remotion`.
...
For displaying multiple elements one after another with a transition in between, use the `TransitionSeries` component from `@remotion/transitions`.
...
Remotion needs all React code to be deterministic. Therefore it is forbidden to use the `Math.random()` API. If randomness is requested, use the `random()` function from `remotion` and pass a static seed to it. The `random` function returns a number between 0 and 1.
...
`interpolate()` accepts a number and two arrays of numbers. The first argument is the value to animate. The first array is the input range, the second is the output range. The fourth argument is optional, but code should add `extrapolateLeft: 'clamp'` and `extrapolateRight: 'clamp'` by default. The function returns a number between the first and second array.

## useVideoConfig()

If the `fps`, `durationInFrames`, `height` or `width` of the composition are required, use the `useVideoConfig()` hook from `remotion`.
...
UI components in Remotion are fundamentally different from normal interactive React components.
...
1. **State Management** — Remotion: use `useCurrentFrame()` to drive animations. Normal React: use `useState()` for interactive state.
2. **Animations** — Remotion: use `interpolate()` or `spring()` based on frame number. Normal React: CSS transitions, animation libraries, or requestAnimationFrame.
3. **User Input** — Remotion: no user input; all props must be passed at composition time. Normal React: handle clicks, form inputs, gestures, etc.
4. **Effects** — Remotion: avoid `useEffect`; calculations should be pure based on frame. Normal React: use `useEffect` for side effects and subscriptions.
